...
def analy():
    ...
    outLat = air_db.execute("""
        ...
        ,STATION_CODE,STATION_NAME,STATION_TYPE,POINT_CODE,POINT_NAME
         FROM t_air_point_hour where MONITOR_TIME>DATE_SUB(NOW(), INTERVAL 3 hour)  and STATION_TYPE = 5
        """).all()
    air_db.commit()
    ...
    for out in outLat:
        ...
        # print(result)
        ## 3-修改空气质量数据
        update_sql = "update t_air_point_hour set " \
                     + "aqi=" + str(result['aqi']) \
    ...
    ## 获取空气质量数据
    outLat = air_db.execute("""
        SELECT ID,STATION_CODE,STATION_NAME,POINT_CODE,POINT_NAME,MONITOR_TIME,O3
        FROM t_air_point_hour WHERE MONITOR_TIME>DATE_SUB(NOW(), INTERVAL 1 hour)
    """).all()
    print("------O3_8h update size ：" + str(len(outLat)) + "------")
    for out in outLat:
        selectsql = "select STATION_CODE, ANY_VALUE(ROUND(AVG(O3),2)) from t_air_point_hour where MONITOR_TIME>DATE_SUB(NOW(), INTERVAL 8 hour)" \
                    " AND STATION_CODE= :code GROUP BY STATION_CODE"
        eighthourdata = air_db.execute(selectsql, {"code": out[1]})
        fetchOne = eighthourdata.fetchone()
        print(fetchOne[1])
        if (fetchOne[1] == '' or fetchOne[1] == None
        ):
            continue
        update_sql = "update t_air_point_hour set " \
                     + "o3_8h=" + str(fetchOne[1]) \
                     + " where id='" + str(out[0]) + "'"
        print(update_sql)
        outLat = air_db.execute(update_sql)
        air_db.commit()
